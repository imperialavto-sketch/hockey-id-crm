...
function ReportRow({
  ...
  item: CreatedReportListItem;
  onOpen: () => void;
  isFirst: boolean;
}) {
  const title = item.title?.trim() || "Отчёт";
...
export default function CreatedReportsScreen() {
  const router = useRouter();
  const [items, setItems] = useState<CreatedReportListItem[]>([]);
  ...
  return (
    <ScreenContainer>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        <Text style={styles.heroEyebrow}>Отчёты</Text>
        <Text style={styles.heroTitle}>Созданные отчёты</Text>
        <Text style={styles.heroSub}>
          Отчёты, созданные вручную и из голосовых заметок (read-only просмотр).
        </Text>

        {items.length === 0 ? (
          <SectionCard elevated style={styles.emptyCard}>
            <Text style={styles.emptyTitle}>Пока нет отчётов</Text>
            <Text style={styles.emptyText}>
              Создайте отчёт из голосовой заметки или вручную — он появится здесь.
            </Text>
            <PrimaryButton
          ...
          <SectionCard elevated style={styles.listCard}>
            {items.map((it, idx) => (
              <ReportRow
                ...
                item={it}
                isFirst={idx === 0}
                onOpen={() => router.push(`/created-reports/${it.id}`)}
              />
            ))}
          </SectionCard>
        )}
...
const styles = StyleSheet.create({
  ...
  listCard: { borderLeftWidth: 4, borderLeftColor: theme.colors.success },
  ...
  rowMeta: { flexDirection: "row", alignItems: "center", gap: theme.spacing.sm, flexWrap: "wrap", marginTop: 6 },
