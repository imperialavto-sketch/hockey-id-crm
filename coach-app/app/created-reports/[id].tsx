...
import {
  ...
  type CreatedReportDetail,
} from "@/services/createdReportsService";

export default function CreatedReportDetailScreen() {
  ...
  const [report, setReport] = useState<CreatedReportDetail | null>(null);
    ...
    return (
      <ScreenContainer>
        <View style={styles.center}>
          <Text style={styles.title}>Отчёт не найден</Text>
  ...
  const showVoiceProvenance = hasVoiceNoteLink(report.voiceNoteId);
  const linkedVoiceNoteId =
    showVoiceProvenance && typeof report.voiceNoteId === "string" ? report.voiceNoteId.trim() : "";

  return (
    <ScreenContainer>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        <Text style={styles.eyebrow}>Отчёт</Text>
        <Text style={styles.title}>Просмотр</Text>
        <Text style={styles.meta}>{formatVoiceDateTimeFullRu(report.createdAt)}</Text>
        ...
        <SectionCard elevated style={styles.card}>
          ...
          <Text style={styles.body}>{report.title}</Text>
          {report.playerName ? (
            <Text style={styles.bodyMuted}>Игрок: {report.playerName}</Text>
          ) : null}
        </SectionCard>

        <SectionCard elevated style={styles.card}>
          <Text style={styles.kicker}>Текст</Text>
...
const styles = StyleSheet.create({
  ...
  card: { marginBottom: theme.spacing.lg, borderLeftWidth: 4, borderLeftColor: theme.colors.success },
  kicker: {
    ...theme.typography.caption,
    color: theme.colors.textMuted,
    textTransform: "uppercase",
