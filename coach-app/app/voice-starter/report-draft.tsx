...
import {
  createReport,
  voiceStarterToCreateReportInputFromEditedText,
...
export default function VoiceStarterReportDraftScreen() {
  ...
  const reportText = payload ? formatVoiceStarterForReportDraft(payload) : "";
  const destinationContext = payload ? buildVoiceStarterDestinationContext(payload) : null;

  useEffect(() => {
    setComposerText(reportText);
  }, [reportText]);
  ...
  useEffect(() => {
    ...
    getVoiceCreationMemoryForNote(voiceNoteId).then((memory) => {
      if (cancelled) return;
      setDuplicateHint(Boolean(memory.report_draft));
    });
    return () => {
      cancelled = true;
    };
  ...
  const handleCreateOnServer = useCallback(async () => {
    ...
    if (!submittedText) {
      Alert.alert("Добавьте текст", "Перед сохранением заполните черновик отчёта.");
      return;
    }
    submitLockRef.current = true;
    setSubmitting(true);
    try {
      const input = voiceStarterToCreateReportInputFromEditedText(payload, composerText);
      ...
      const voiceNoteId = payload.originVoiceNoteId?.trim();
      if (voiceNoteId) await markVoiceCreationForNote(voiceNoteId, "report_draft");
    ...
    return (
      <ScreenContainer>
        <View style={styles.center}>
          ...
          <Text style={styles.loadingEyebrow}>Заметка → отчёт</Text>
          <Text style={styles.title}>Собираем черновик отчёта</Text>
          <Text style={styles.sub}>Подставляем структуру из голосовой заметки…</Text>
        </View>
      </ScreenContainer>
    );
  }
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <StaggerFadeIn preset="snappy" revealKey={payload.id} delay={0}>
          <SectionCard elevated style={styles.heroCard}>
            ...
            <Text style={styles.title}>Черновик отчёта подготовлен</Text>
            <Text style={styles.sub}>
              Заметка уже разложена в структуру отчёта. Осталось проверить формулировки и сохранить — тогда запись
              появится в списке отчётов.
            </Text>
            {payload.aiProcessed ? (
            ...
            {duplicateHint ? (
              <Text style={styles.duplicateHint}>
                По этой заметке уже создавали отчёт. Можно обновить формулировки и сохранить новый вариант.
              </Text>
            ) : null}
          </SectionCard>
          ...
          <SectionCard elevated style={styles.contextCard}>
            ...
            <Text style={styles.summaryText}>{destinationContext?.summary ?? "Краткая выжимка пока недоступна."}</Text>
            {destinationContext?.highlights.length ? (
              ...
              <Text style={styles.emptyText}>Ключевые пункты не выделены. Используйте текст черновика ниже.</Text>
            )}
            <View style={styles.metaRow}>
              ...
              <Text style={styles.metaChip}>
                Контекст: {destinationContext?.sessionLabel ?? "без уточнения"}
              </Text>
            </View>
          </SectionCard>

          <SectionCard elevated style={styles.editorCard}>
            <Text style={styles.kicker}>Текст отчёта</Text>
            <Text style={styles.editorHint}>
              Это почти готовый отчёт — отредактируйте при необходимости, затем сохраните на сервере.
            </Text>
            <TextInput
              ...
              placeholder="Текст черновика отчёта…"
          ...
          <SectionCard elevated style={styles.actionsCard}>
            <Text style={styles.nextStepHint}>
              Следующий шаг: проверьте текст и нажмите кнопку сохранения — отчёт будет создан на сервере.
            </Text>
            <PrimaryButton
              title={
                submitting
                  ? "Создание…"
                  : duplicateHint
                    ? "Сохранить новый вариант отчёта"
                    : "Сохранить отчёт на сервере"
              }
            ...
            <PrimaryButton
              ...
              variant="outline"
              onPress={async () => {
                await Clipboard.setStringAsync(composerText.trim() || reportText);
