    ...
    >
      <View style={styles.rowContent}>
        ...
        <View style={styles.badge}>
          <Text style={styles.badgeText}>Готов</Text>
        </View>
...
export default function ReportsScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      getWeeklyReadyReports()
        ...
        .catch((err) => {
          setReports([]);
          setError(isAuthRequiredError(err) ? "Требуется авторизация" : "Не удалось загрузить отчёты");
        })
  ...
  const countLabel =
    reports.length > 0
      ? `${reports.length} ${pluralReport(reports.length)}`
      : "Нет готовых отчётов";
      ...
      <ScreenContainer contentContainerStyle={styles.content}>
        <StaggerFadeIn delay={0}>
          <View style={styles.hero}>
            <Text style={styles.eyebrow}>Отчёты недели</Text>
            <Text style={styles.title}>Готовые отчёты</Text>
            ...
            <PrimaryButton
              ...
              onPress={() => {
                ...
                getWeeklyReadyReports()
                  ...
                  .catch((err) => setError(isAuthRequiredError(err) ? "Требуется авторизация" : "Не удалось загрузить отчёты"))
                  .finally(() => setLoading(false));
              }}
    ...
    <ScreenContainer contentContainerStyle={styles.content}>
      ...
      <StaggerFadeIn delay={15}>
        <View style={styles.section}>
          {reports.length === 0 ? (
            <SectionCard elevated style={styles.reportCard}>
              <Text style={styles.emptyTitle}>
                {isEndpointUnavailable(COACH_ENDPOINTS.REPORTS_WEEKLY)
                  ? 'Модуль отчётов пока не подключён'
                  : 'Пока нет готовых отчётов'}
              </Text>
              <Text style={styles.emptyText}>
                {isEndpointUnavailable(COACH_ENDPOINTS.REPORTS_WEEKLY)
                  ? 'Когда сервер будет готов, здесь появятся отчёты'
                  : 'Запишите тренировку'}
              </Text>
              {isEndpointUnavailable(COACH_ENDPOINTS.REPORTS_WEEKLY) ? (
                <PrimaryButton
            ...
            <SectionCard elevated style={styles.reportCard}>
              {reports.map((r, i) => (
                <ReportRow
                  ...
                  item={r}
                  isFirst={i === 0}
                  onOpen={() => router.push(`/player/${r.playerId}/report`)}
                />
              ))}
            </SectionCard>
          )}
