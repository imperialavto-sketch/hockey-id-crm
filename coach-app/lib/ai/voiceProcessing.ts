import type { VoiceProcessingInput, VoiceProcessingResult } from "./types";
...
/** Делит текст на предложения (RU + латиница, грубое но достаточное для MVP). */
...
/**
 * Ищет предложения/фрагменты, где встречается маркер (по предложениям).
 */
...
function buildParentDraft(
  ...
  const greeting = name ? `Здравствуйте! Кратко по ${name} после тренировки.` : "Здравствуйте! Кратко по итогам тренировки.";
...
/**
 * Локальная обработка без внешнего API.
 * Замените тело на вызов провайдера, когда появится реальный AI.
 */
function processVoiceNoteLocal(input: VoiceProcessingInput): VoiceProcessingResult {
...
/**
 * Публичная точка входа. Сейчас — умный mock; позже: `return remoteProvider(input)`.
 */
export async function processVoiceNote(
  ...
  return processVoiceNoteLocal(input);
}
