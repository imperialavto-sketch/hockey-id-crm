...
**Не проверялись в smoke-test:** POST /api/subscription, POST /api/subscription/cancel, POST /api/notifications/:id/read (нужны доп. данные).
...
| docs/SMOKE_TEST_BUILD_FIX_REPORT.md | NEW — этот отчёт |
...
Настроить окружение для subscription и ai-analysis (Stripe, OpenAI, переменные) и повторно прогнать smoke-test для этих endpoints.
