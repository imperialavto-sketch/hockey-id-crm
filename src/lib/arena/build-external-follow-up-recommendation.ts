...
const SOURCE_NOTE_LOAD =
  "Учтены расписание команды за последние семь дней и недавняя активность дополнительного контура.";
...
function deferDueToLoadView(load: PlayerLoadSnapshot): ExternalFollowUpRecommendationView {
  const points: string[] = [
    "Арена учитывает текущую тренировочную нагрузку и недавнюю активность дополнительного контура.",
  ];
  if (load.weeklySessions >= 4) {
    points.push(
      "Сейчас видно, что в расписании команды за неделю уже несколько тренировок."
    );
  }
  ...
  if (points.length < 3 && load.externalReportsLast2Days >= 2) {
    points.push("За короткий промежуток накопилось несколько внешних отчётов — пауза уместна.");
  }
  if (points.length < 3) {
    points.push("Сейчас лучше не увеличивать дополнительный контур — опирайтесь на школу и наблюдение.");
  }
  return {
    type: "defer_due_to_load",
    title: "Арена предлагает не увеличивать нагрузку",
    summary:
      "Сейчас у игрока уже есть заметная текущая нагрузка в основном контуре и недавняя активность дополнительного источника. Ещё одна дополнительная сессия сейчас может быть лишней: лучше не наращивать объём и вернуться к вопросу позже, опираясь на школу и наблюдение.",
    actionLabel: null,
    deferLabel: null,
