/**
 * GET /api/coach/reports/player/[id]
...
function canAccessPlayer(accessibleIds: string[] | null, playerId: string): boolean {
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canAccessPlayer(accessibleIds, id)) {
  ...
  try {
    ...
    const improvementAreas = Array.isArray(draft.improvementAreas) ? draft.improvementAreas : [];
    ...
    if (snapshots?.skills && typeof snapshots.skills === "object") {
      ...
      if (withScore.length > 0) {
        avgScore = Math.round(withScore.reduce((a, s) => a + (s.score ?? 0), 0) / withScore.length);
      }
    }

    const recommendations =
      focusSkills.length > 0 ? [`Сфокусироваться на ${focusSkills.join(", ")}.`] : improvementAreas.slice(0, 2);
    ...
    console.error("GET /api/coach/reports/player/[id] failed:", error);
    return NextResponse.json(
      {
